...
class FirstBlock(nn.Module):
    def __init__(self, in_channel,output_channel,stride=1):
        ...
        self.conv1 = nn.Conv2d(in_channel, output_channel, kernel_size=3, stride=stride,padding=1, bias=False)
        ...
        self.conv3 = nn.Conv2d(in_channel, output_channel, kernel_size=1, stride=stride,padding=0, bias=False)
...
class TsingNet(nn.Module):
    def __init__(self, num_classes=1000,input_channel=3,loss_type=False):
        ...
        self.conv1 = nn.Conv2d(input_channel, 8, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
def tsing_net(num_classes=1000, input_channel=3,loss_type=False):
    ...
    model = TsingNet(num_classes=num_classes,input_channel=input_channel,loss_type=loss_type)
    return model